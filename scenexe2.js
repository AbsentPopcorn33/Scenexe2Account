    ...
    app = function ($, e) {
      ...
      "/tankData.json" === a
        ...
        : (e.writeHead(200, { "Content-Type": "text/html; charset=utf-8" }),
          e.end(
            '<script>document.write(`<a style="font-family:monospace" href="${\'https://scenexe2.io?s=\' + new URL(location.href).host}">connect to private server</div>`)</script>'
          ));
    },
    httpServer =
        ...
        if (
          ...
          for (let r = $.newTanks.length - 1; r >= 0; r--) {
            ...
            if (
              ...
              console.log(
                "dim",
                `name:${d[0].name} score:${Math.round(d[0].score)} dim:${
                  $.name
                }`
              ),
              (d[1].data.waiting = !1),
  ...
  server.on("connection", ($, e) => {
    ...
    clients.push($),
      ...
      $.on("message", (e) => {
        ...
        //        BlockLog =
        if (
          !["d", "controlPosition", "direction", "firing", "ping"].includes(
            game.codes.send[t[0]]
          )
        ) {
            ...
            if ("login" === n) {
              ...
              if (LoginSuccess) {
                ($.accountNameParsed = t[1][0]),
                  ($.accountName = $.accountNameParsed.toLowerCase()),
                  console.log($.accountNameParsed, $.accountData),
                  console.log(
                ...
                $.send(
                  pack([
                    game.codes.recieve["eval"] +
                      "alert('Failed : username or password incorrect')",
                  ])
                );
              }
              return;
            }
            if ("token" === n) console.log("test");
              ...
              if ("/" === y[0]) {
                ...
                if ($.data.tank) {
                  ...
                  w &&
                    console.log(
                      "command used",
                      $.data.tank.dim ? $.data.tank.dim.name : "undefined",
                      $.accountNameParsed,
                      y
                    ),
                    w &&
